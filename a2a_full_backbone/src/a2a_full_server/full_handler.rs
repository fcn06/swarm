...
///
#[derive(Clone)]
pub struct FullAgentHandler {
    /// Task storage that implements all the business capabilities
    storage: Arc<InMemoryTaskStorage>,
    _config:AgentFullConfig,
    full_agent: Arc<Mutex<FullAgent>>,
}

impl FullAgentHandler {
    /// Create a new simple agent handler
    pub fn new(_storage: InMemoryTaskStorage, config:AgentFullConfig, full_agent: FullAgent) -> Self {
        println!("Creating FullAgentHandler");
        Self {
            storage: Arc::new(InMemoryTaskStorage::new()),
            _config:config,
            full_agent: Arc::new(Mutex::new(full_agent)),
        }
    }
...
#[async_trait]
impl AsyncMessageHandler for FullAgentHandler {
    async fn process_message<'a>(
        ...
        let mut full_agent = self.full_agent.lock().await;
        let response: ExecutionResult = full_agent.handle_user_request(message.clone()).await;


        // Convert the message Back to A2A Message
        let llm_response_from_full_agent = Message_Llm {
            role: "tool".to_string(), // Or appropriate role based on ExecutionResult
            content: Some(response.output.clone()),
            tool_call_id: None,
            tool_calls:None
        };
        let response_message = self.llm_message_to_a2a_message(llm_response_from_full_agent)?;
...
#[async_trait]
impl AsyncTaskManager for FullAgentHandler {
...
#[async_trait]
impl AsyncNotificationManager for FullAgentHandler {
...
#[async_trait]
impl AsyncStreamingHandler for FullAgentHandler {
