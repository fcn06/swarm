...
pub async fn evaluate_agent_output(log_data: AgentLogData) -> Result<EvaluatedAgentData> {
    dotenv::dotenv().ok();

    let llm_api_key = env::var("LLM_API_KEY")?;
    let llm_endpoint = env::var("LLM_ENDPOINT")?;
    let llm_model = env::var("LLM_MODEL").unwrap_or_else(|_| "gemini-pro".to_string());
...
```"#,
    ...
    let llm_interaction = ChatLlmInteraction::new(llm_endpoint, llm_model, llm_api_key);
    
    let response = llm_interaction.call_api_simple("user".to_string(), prompt).await?;
