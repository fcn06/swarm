...
pub struct OrchestrationAgentConfig {
    ...
    pub agent_llm_url: String, // This is the LLM that will manage interactions with Full agent. LLM_FULL_API_KEY is connected to this one
    pub agent_mcp_config_path: Option<String>, // The path of the configuration for the MCP runtime
    pub agent_agents_references:Option<Vec<AgentReference>>, // List all agents connected to full agents
...
impl AgentConfig for OrchestrationAgentConfig {
    ...
    fn agents_references(&self) -> Option<Vec<AgentReference>> { self.agent_agents_references.clone()  }
}