...
pub struct AgentServer {
    ...
    agent:Agent, // Agent of type either BasicAgent or OrchestrationAgent
}
...
impl AgentServer {
    ...
    pub async fn new(agent_a2a_config: AgentA2aConfig) -> anyhow::Result<Self> {
        ...
        let agent= Agent::new(agent_a2a_config.clone()).await?;
    ...
    fn create_in_memory_storage(&self) -> InMemoryTaskStorage {
            ...
            S: AsyncTaskManager + AsyncNotificationManager + Clone + Send + Sync + 'static,
        {
            ...
            let message_handler =
                AgentHandler::with_storage(self.agent.clone(),storage.clone());
